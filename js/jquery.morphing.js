(function ($, window, document, undefined) {
  ...
  function genCanvas() {
    ...
    if ($img.length > 0) {
      ...
      this.$el.css("backgroundColor", "transparent").append($(canvas));
      return {
        img: false,
        canvas: canvas,
        numVert: this.numVert,
  ...
  function MorphingCircle(spec) {
    ...
    this.init = function () {
      ...
      for (var i = 0; i < numVert; i++) {
        ...
        points.push(
          new AnchorPoint2D(
            radius * Math.cos(rad) + x,
            radius * Math.sin(rad) + y
          )
        );
      }
    };
  ...
  Morphing.prototype = {
    init: function () {
      this.imgLoad(this.$el.find("img").attr("src")).done(
        $.proxy(this.motion, this)
      );
    },
